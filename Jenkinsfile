  ...
  stages {
    stage('Build with Kaniko') {
      steps {
        ...
        docker run -v $(pwd):/usr gcr.io/kaniko-project/executor:latest --insecure --dockerfile=Dockerfile --context= `pwd` --destination=registry.me:5000/nodejs_helloworld:5.1
      }
    }
  }
}
